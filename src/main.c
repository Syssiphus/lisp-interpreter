/*
 * Bootstrap Scheme - a quick and dirty scheme interpreter.
 * Copyright (C) 2010 Peter Michaux (http://peter.michaux.ca/)
 *
 * Retyped for exercise and study by B.Paschen (bpaschen@googlemail.com).
 *
...
typedef enum {
               FIXNUM         /* Fixed number type   */
             , BOOLEAN        /* Boolean type        */
             , CHARACTER      /* Character type      */
             , STRING         /* String type         */
             , THE_EMPTY_LIST /* Empty list type     */
             , PAIR           /* Pair type           */
             , SYMBOL         /* Symbol type         */
             , PRIMITIVE_PROC /* Primitive procedure */
             , COMPOUND_PROC  /* Compound procedure  */
             , INPUT_PORT     /* Input stream        */
             , OUTPUT_PORT    /* Output stream       */
             , EOF_OBJECT     /* The EOF object      */
} object_type;
...
void dbg_write(const char *msg, object *obj);

/* No GC, so truely unlimited extend */
...
object *add_proc(object *arguments)
{
    ...
    {
        /* FIXME: missing check for argument type */
...
object *is_eq_proc(object *arguments)
{
    ...
    {
        case FIXNUM:
            return (obj1->data.fixnum.value == obj2->data.fixnum.value)
                ? true : false;
            break;
        case CHARACTER:
            return (obj1->data.character.value == obj2->data.character.value)
                ? true : false;
            break;
        case STRING:
            return (strcmp(
                        obj1->data.string.value, 
                        obj2->data.string.value) == 0)
                ? true : false;
            break;
        case PAIR:
            if(is_true(is_eq_proc(cons(car(obj1), 
                                cons(car(obj2), the_empty_list)))))
            {
                return is_eq_proc(cons(cdr(obj1),
                            cons(cdr(obj2), the_empty_list)));
            }
            else
            {
                return false;
...
{
    write(stdout, make_string("Exiting. Bye."));
    putc('\n', stdout);
    exit(0);
}
...
object *close_output_port_proc(object *arguments)
{
    ...
    if (result == EOF)
    {
        fprintf(stderr, "Couldn't close output port\n");
        exit(1);
    }
    return ok_symbol;
}
...
object *setup_environment(void)
{
    ...
    initial_env = extend_environment(the_empty_list, 
            the_empty_list, the_empty_environment);
    return initial_env;
}
...
void peek_expected_delimiter(FILE *in)
{
    ...
    {
        fprintf(stderr, "character not followed by a delimiter\n");
        exit(1);
    }
}
...
object *read_pair(FILE *in)
{
    ...
    if (c == '.')
    {
        ...
        if (c !=')')
        {
            fprintf(stderr, "where was the railing right paren?\n");
            exit(1);
        }
...
#define BUFFER_MAX 1000
    ...
    else if (c == '"')
    {
        ...
        while ((c = getc(in)) != '"')
        {
            ...
            /* subtract 1 to leave space for '\0' string terminator */
            if (i < BUFFER_MAX - 1)
            {
                buffer[i++] = c;
            }
    ...
    {
        fprintf(stderr, "bad input. Unexpected '%c' (Dec: %d)\n", c, c);
        exit(1);
    }
...
tailcall:
    ...
    else if (is_application(exp))
    {
        ...
        {
            /* prepare the function call and environment */
